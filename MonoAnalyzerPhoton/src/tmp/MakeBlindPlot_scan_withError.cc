...
void MakeBlindPlot_scan_withError(int Unblind, string year){
...
//				+pow(Actual->GetBinError(3,2),2)+pow(Expected->GetBinError(3,2),2)));
	...
	if(ChiSquare[i] < 3){
	    ofstream fout("output/csv_file/ABCD_"+year+"_"+LooseX+"_"+LooseY+"_noErr.csv"); 
...
void ChiSquare_scan(double *x_,double *y_, int nloop, vector<double> ChiSquare,string year){
        ...
        ofstream Scan_result(("output/csv_file/ABCD_scan"+year+"_noErr_branch2.csv").c_str()); 
