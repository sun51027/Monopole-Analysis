...
void doAnalysis(TChain *Mono,TH2F *Plot,string year){
    ...
    for(int ev=0; ev<1000000/*Mono->GetEntries()*/; ev++){
...
void MakeBlindPlot_onlyPlot(int Unblind, string year){

    TCanvas *c = new TCanvas("c","",800,600);
//    TChain *Mono  = new TChain("monopoles");
    TFile *fin = new TFile("output/BlindAnalysis_ABCD_Jack.root");
    TDirectory *dc = (TDirectory*)fin->Get("HLT_Photon");
    dc->cd();  
    TH2D* Plot = (TH2D*)dc->Get("Blinded_ABCD");
