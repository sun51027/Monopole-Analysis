...
//		 To see the overlap between PhotonCollection and PFcand
...
enum ProfileName{
	...
	PFMETvsOnlyOnePID,
	PFMETvsOneCandOnePID,
	PFMETvsTwoPID
};
static const unsigned nPlot = 40U;
static const unsigned nProfile = 3U;
class PlotSet
{
public:
  PlotSet(){
	plots_.resize(nPlot);
  	profile_.resize(nProfile);
  }
  ~PlotSet(){}
  void CreatPlot(const PlotName pn, TH1* h){ 
	plots_[pn] = h;
  }
  void CreatProfile(const ProfileName f, TProfile* h){ 
	profile_[f] = h;
  }
  TH1 * GetPlot(const PlotName pn){ return plots_[pn]; }
  TProfile *GetProfile(const ProfileName f){ return profile_[f]; }
  void WritePlot(){
	cout<<"I get into WritePlot"<<endl;
	for(int pn=0;pn<nPlot;pn++){
		TH1 *h = plots_[pn];
		if(h){
		  	h->Write();
			cout<<"I'm writing TH1"<<endl;
		}
	}
	for(int f=0;f<nProfile;f++){
		TProfile *Pf = profile_[f];
		if(Pf){
			  Pf->Write();
			cout<<"I'm writing profile"<<endl;
		}
	}	
  }
private:
...
public:
  Photon(){}
  Photon(double pho_eta, double pho_phi, double pho_pt,double pho_E):pho_eta_(pho_eta),pho_phi_(pho_phi),pho_pt_(pho_pt),pho_E_(pho_E){}
  Photon(const Photon &mc):pho_eta_(mc.pho_eta_),pho_phi_(mc.pho_phi_),pho_pt_(mc.pho_pt_),pho_E_(mc.pho_E_){}
  double pho_eta_;
  double pho_phi_;
  double pho_pt_;
  double pho_E_;
  ...
  bool operator<(const Photon &mc)const{
   ...
   else if(pho_pt_==mc.pho_pt_){
        if(pho_E_>mc.pho_E_) return true;
        else return false;
        }
    else return false;
  }
...
public:
  ...
  MonoCuts(const string &trName,TFile *openFile):trigName_(trName){
	...
	x.CreatProfile(PFMETvsOnlyOnePID	,new TProfile("PFMETvsOnlyOnePID","",6,0,6,0,1100));
	x.CreatProfile(PFMETvsOneCandOnePID	,new TProfile("PFMETvsOneCandOnePID","",6,0,6,0,1100));
	x.CreatProfile(PFMETvsTwoPID		,new TProfile("PFMETvsTwoPID","",6,0,6,0,1100));
  }
...
//	cout<<"enter doAnalysis successfully"<<endl;	
	...
	//Cut for this study(PFMET vs monopole)
	...
	//every events have at most 2 monopoles, that is only one monopole or antimonopole
	...
	for(int j=0;j<Shower.size();j++){
			...
			//To avoid the too large energy difference between PF cand and recoMono
			//But it is not influnt my conclusion too much (I think...
			double Energydiff=0;
			Energydiff = abs(Shower[j].e55_-HighPtParticleflow[i].pf_E_);
			if( R  < 0.15 )//&& Energydiff < 100)
			{
	...
	for(int j=0;j<AntiShower.size();j++){
			...
			double Energydiff=0;
			Energydiff = abs(AntiShower[j].e55_-HighPtParticleflow[i].pf_E_);
			if( R  < 0.15 )//&& Energydiff < 100)
			{
	...
	//Photon COllection(use to compare with the PF algorithm)
	for(int j=0;j<Shower.size();j++){
		for(int i=0;i<HighPtPhoton.size();i++){
        		double photonMono_deltaR = 0;
				  ...
				  photonLike++;
				  nPhotonlike++;
				  MatchedPhoton.push_back(HighPtPhoton[i]);		
			}
		}
	}
	for(int j=0;j<AntiShower.size();j++){
		for(int i=0;i<HighPtPhoton.size();i++){
        		double photonMono_deltaR = 0;
                        photonMono_deltaR = sqrt(pow(AntiShower[j].eta_-HighPtPhoton[i].pho_eta_,2)+
                                                 pow(AntiShower[j].phi_-HighPtPhoton[i].pho_phi_,2));
                        if(photonMono_deltaR < 0.15 ){
				  photonLike++;
				  nPhotonlike++;
				  MatchedPhoton.push_back(HighPtPhoton[i]);		
	...
	cout<<setw(5)<<" | Photon          "<<photonLike<<" |"<<endl;
	cout<<setw(5)<<" ---------------------"<<endl;		
	...
	if(Shower.size()==1 && AntiShower.size() == 1 && pfMono==1 && pfAmon==1){
		...
		if(pfAntiUnknown[0].pf_pdgId_ == 7) x.GetPlot(PFMETvsTwo7EMEnergyinHF)->Fill(PFMET_pt);
  		...
  		x.GetProfile(PFMETvsTwoPID)->Fill(double(pfAntiUnknown[0].pf_pdgId_),PFMET_pt);

	  //Unexpected cases,MET should be small but found some largeMET cases
	  //Jack said it is normal tail for MET....
	  if(PFMET_pt > 300){
		Print();
		WeirdTwoPfEvent++;
  	   }
		...
		if(pfMono!=0){
			...
			x.GetProfile(PFMETvsOneCandOnePID)->Fill(double(pfUnknown[0].pf_pdgId_),PFMET_pt);
		...
		if(pfAmon!=0){
			...
			x.GetProfile(PFMETvsOneCandOnePID)->Fill(double(pfAntiUnknown[0].pf_pdgId_),PFMET_pt);
		}
	} 
		...
		x.GetProfile(PFMETvsOnlyOnePID)->Fill(double(pfUnknown[0].pf_pdgId_),PFMET_pt);
	}
		...
		x.GetProfile(PFMETvsOnlyOnePID)->Fill(double(pfAntiUnknown[0].pf_pdgId_),PFMET_pt);
	} 
 ...
 void Print(){
  ...
  void PrintPhoton(){
       if(MatchedPhoton.size()!=0){
          for(int j=0;j<MatchedPhoton.size();j++){
                cout<<j+1<<" Matched Photon Info ----------------"<<endl;
                cout<<"pho  eta "<<MatchedPhoton[j].pho_eta_<<endl;
                cout<<"pho  phi "<<MatchedPhoton[j].pho_phi_<<endl;
                cout<<"pho   pt "<<MatchedPhoton[j].pho_pt_<<endl;
          }
	}
  } 
...
private:
  ...
  const double MonoCuts::e55Cut_ = 50;
  const double MonoCuts::f15Cut_ = 0.95;
  const double MonoCuts::SwissCrossCut_ = 0.95;
  const double MonoCuts::f51Cut_ = 0.85;
  const double MonoCuts::PFMETCut_ = 0;//we only look to high MET case
  const double MonoCuts::photonCut_ = 0;
  const double MonoCuts::electronCut_ = 0;
  const double MonoCuts::particleflowCut_ = 50;//the lowest require for PF cut
void MonoAnalyzerPFMET_PF()
{
	TFile *oFile = new TFile("Monopole_PFMET_PF_1000.root","recreate");

	TFile *fin = new TFile("../MonoNtuple2018/MonoNtuple2018_MC_1000_0804.root");
