//
//	MonoAnalyzerPFMET.cc
//	Created by  Shih Lin
//	
//	Analysis code for PFMET trigger
//	To understand the relation between monopole and MET
//	root -l -q MonoAnalyzerPFMET.cc
//
#include "iostream"
...
//	x.CreatPlot(pfUnknownLikePt,new TH1D("pfUnknownLikePt","",30,0,800));
	...
	x.CreatProfile(profile_2PF,new TProfile("profile_2PF","",6,0,6));
        ...
        x.CreatProfile(profile_1PFonly,new TProfile("profile_1PFonly","",6,0,6));
  }
  ...
  void doAnalysis(vector<MonoCandidate> &cand, vector<Photon> & pho, vector<Electron> &ele,vector<Particleflow> &pf, unsigned nCandidates,unsigned nPhoton, unsigned nElectron, unsigned nParticleflow, bool TRG, unsigned ev, double PFMET_pt, double PFMET_phi)
  {
	...
	if(CutFlowCand_TRG.size()>0) count_TRG++;
	...
	//to look monopole correspond to what particle in the PF, it could be anything in the PF
