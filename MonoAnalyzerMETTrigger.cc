...
public:
  ...
  MonoCuts(const string &trName,TFile *openFile):trigName_(trName){
	...
	x.CreatPlot(MET,new TH1D("MET","",100,-1,1200));
...
//	 cutName_[1] = "Energy_";
	...
	z.CreatPlot(RZcurv,new TH1D((cutn1name+"RZcurv").c_str(),"",100,-0.01,0.01));
        z.CreatPlot(MET,new TH1D((cutn1name+"MET").c_str(),"",100,0,800));
	...
	string cutflowName = "Flow_HLT_"+cutName_[c];
        ...
        y.CreatPlot(MET,new TH1D((cutflowName+"MET").c_str(),"",100,0,800));
...
*/
	...
	if(CutFlowCand_TRG.size()>0) 
	{
		...
		x.GetPlot(MET)->Fill(CutFlowCand_TRG[0].MET_pt_);
	...
	if(CutFlowCand_Qual.size()>0) 
	{
		...
		x.GetPlot(MET)->Fill(CutFlowCand_Qual[0].MET_pt_);
	...
	if(CutFlowCand_MET.size()>0)
	{
		...
		x.GetPlot(MET)->Fill(CutFlowCand_MET[0].MET_pt_);
	...
	if(CutFlowCand_F51.size()>0)
	{
		...
		x.GetPlot(MET)->Fill(CutFlowCand_F51[0].MET_pt_);
	...
	if(CutFlowCand_Dedx.size()>0)
	{
		...
		x.GetPlot(MET)->Fill(CutFlowCand_Dedx[0].MET_pt_);
	...
	if(N1CutCand_Qual.size()>0) 
	{
	        ...
	        z.GetPlot(RZcurv)->Fill(N1CutCand_Qual[0].rzp2_);
       		z.GetPlot(MET)->Fill(N1CutCand_Qual[0].MET_pt_);
	...
	if(N1CutCand_MET.size()>0)
	{
	        ...
	        z.GetPlot(RZcurv)->Fill(N1CutCand_MET[0].rzp2_);
       		z.GetPlot(MET)->Fill(N1CutCand_MET[0].MET_pt_);
	...
	if(N1CutCand_F51.size()>0)
	{
	        ...
	        z.GetPlot(RZcurv)->Fill(N1CutCand_F51[0].rzp2_);
       		z.GetPlot(MET)->Fill(N1CutCand_F51[0].MET_pt_);
	...
	if(N1CutCand_Dedx.size()>0)
	{
	        ...
	        z.GetPlot(RZcurv)->Fill(N1CutCand_Dedx[0].rzp2_);
       		z.GetPlot(MET)->Fill(N1CutCand_Dedx[0].MET_pt_);
	...
	if(N1CutCand_TRG.size()>0) 
	{
	        ...
	        z.GetPlot(RZcurv)->Fill(N1CutCand_TRG[0].rzp2_);
       		z.GetPlot(MET)->Fill(N1CutCand_TRG[0].MET_pt_);
...
//		cout<<"starting matching"<<endl;
			...
			cout<<"     MET "<<setprecision(5)<<CutFlowCand_TRG[i].MET_pt_<<endl;
			RealNum++;
			RealEvt=1;
...
/*	cout<<"2 monopole in Ecal     "<<TwoMonoInEcal<<endl;
	cout<<"2 monopole in Tracker  "<<TwoMonoInTracker<<endl;
	cout<<"1 in Tracker 1 in Ecal "<<OneTrkOneEcal<<endl;
	cout<<"1 monopole in Ecal     "<<OneMonoInEcal<<endl;
	cout<<"1 monopole in Tracker  "<<OneMonoInTracker<<endl;
*/
...
private:
  ...
  bool evalMET(MonoCandidate &mc) { return mc.MET_pt_ > METCut_; }
...
//  const double MonoCuts::e55Cut_ = 20;
  const double MonoCuts::METCut_ = 220;
...
void MonoAnalyzerMETTrigger()
{
	cout<<"hello"<<endl;
	TFile *oFile = new TFile("Monopole_2018_METTrigger_4000_MetCut220.root","recreate");
	cout<<"new file line pass"<<endl;

	TFile *fin = new TFile("MonoNtuple2018_MC_4000.root");
